...
class Test_add_key_costs:
    ...
    def test_store_as_np_vectors(
        ...
        stored_costs, stored_freqs = builder._costs_freqs_1d[0]
        assert type(stored_costs).__name__ == "ndarray"
        assert type(stored_freqs).__name__ == "ndarray"
    ...
    def test_store_successive_added_pairs(self) -> None:
        ...
        assert np.array_equal(stored_costs_0, costs_0)
        assert np.array_equal(stored_freqs_0, freqs_0)
    ...
    def test_combine_when_same_freqs(self) -> None:
        ...
        assert len(builder._costs_freqs_1d) == 1
        stored_costs, stored_freqs = builder._costs_freqs_1d[0]
        assert np.array_equal(stored_costs, costs_total)
        assert np.array_equal(stored_freqs, freqs)
...
class Test_add_interkey_costs:
    ...
    def test_store_provided_data(
        ...
        stored_, stored_freqs = builder._costs_freqs_2d[0]
        assert np.array_equal(stored_, [[1, 2], [3, 4]])
