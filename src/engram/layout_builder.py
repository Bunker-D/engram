...
class LayoutBuilder:
    ...
    def fix(self, keys: Iterable[int], chars: Iterable[int]) -> None:
        """
    ...
    def open(self, keys: Iterable[int], chars: Iterable[int]) -> None:
        """
    ...
    def optimal_fill(
        ...
        - `fill_with`: The set of characters to use to fill the keys,
        as identified by their index.
        If instead, `None` is provided (default), then the first *n* usable characters
        (by increasing index) are used.
        (“Usable” ⇔ “unmapped” if `remap = False` (default), “any” else.)
        - `remap`: If `False` (default), already set keys and characters
        are ignored/untouched.
        """
        # TODO Is it needed? It's a shorthand for various step, but is it really useful?
    ...
    def score(self, open_chars_order: Iterable[int] = ()) -> float:
        ...
        If precomputation should be done once for a given set of fixed and opened keys
        and characters, the method can use `self._config_changed` to know when such
        precomputation is needed, as `._config_changed` is set to `True` by
        `.fix(…)`/`.assign(…)` and `.open(…)`. Typically:
        ```python
    ...
    def opened_permutations(self) -> Iterable[tuple[int, ...]]:
        ...
        (By default, each permutation is given as the ordered tuple of the character
        identifying indices.
        If instead for example, the `.score(…)` method uses the indices of the
        characters within `self._opened_chars`, `.opened_permutations()` should be
        overwritten to: `return permutations(range(self._opened_chars))`.)
        """
        return permutations(self._opened_chars)

    def __remove_fixed_from_opened(self) -> None:
    ...
    def __remove_opened_from_fixed(self) -> None:
        for k in self._opened_keys:
            if k in self._fixed:
                del self._fixed[k]


# 💡 If I want to keep the top N best permutations, or those XX% as good as the best,
# 💡 SortedList from https://pypi.org/project/sortedcontainers/ can be useful.
